 ...
 * signed-in customer's meldingen (REPORT-type tickets). Scope is enforced
 * server-side by `scope_tickets_for` BEFORE the `type=REPORT` filter
 * narrows, so no customer param is sent — a caller without customer scope
 * simply gets zero rows. Every row is a melding, so there is intentionally
 * no "type" column. View-first: each row links to the existing
 * `/tickets/<id>` detail page. Mirrors the MyEmployeesPage chrome and the
 * CustomerTicketsPage table.
 */
