...
def apply_proposal_transition(
    ...
    *,
    ...
    # NOTE: The B2 SEND-time cart-coverage + contract-price-floor gate
    # (formerly `_validate_proposal_covers_cart`) was REMOVED on
    # 2026-06-03 by owner decision. A single Extra Work event may
    # legitimately need multiple custom-priced lines and fees that do
    # NOT mirror the submitted cart 1:1. The proposal is the provider's
    # quote and need not match the cart. The only remaining SEND
    # preconditions are: >=1 proposal line and parent EW in
    # UNDER_REVIEW (both checked above).
    ...
    # this intent, a provider SEND finalises pricing and the system
