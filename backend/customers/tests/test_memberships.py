...
class CustomerUserMembershipTests(TenantFixtureMixin, APITestCase):
    def list_url(self, customer_id):
        return f"/api/customers/{customer_id}/users/"

    def detail_url(self, customer_id, user_id):
    ...
    def test_create_requires_customer_user_role_on_target(self):
        self.authenticate(self.super_admin)
    ...
    def test_create_works_with_correct_role(self):
        new_cu = get_user_model().objects.create_user(
            email="brand-new-cu@example.com",
    ...
    def test_delete_works(self):
        ...
        response = self.client.delete(
            self.detail_url(self.customer.id, self.customer_user.id)
        )
    ...
    def test_delete_idempotency_404_on_second_attempt(self):
        self.authenticate(self.super_admin)
        first = self.client.delete(self.detail_url(self.customer.id, self.customer_user.id))
        self.assertEqual(first.status_code, status.HTTP_204_NO_CONTENT)
        second = self.client.delete(self.detail_url(self.customer.id, self.customer_user.id))
