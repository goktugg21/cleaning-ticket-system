...
| 17 | Staff per-building permission granularity | Per-row visibility level (`OWN_ONLY` / `BUILDING_READ` / `BUILDING_READ_AND_ASSIGN`) | `BuildingStaffVisibility` carries only `can_request_assignment` | [`buildings/models.py:52-84`](../../backend/buildings/models.py#L52-L84) | WRONG | **P1** | Add a `visibility_level` enum (or `can_see_all` + `can_assign` booleans) to `BuildingStaffVisibility`; add new osius keys for per-building staff permissions |
