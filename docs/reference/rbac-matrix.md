...
| H-12 | **Committing an invoice is company-level: issue, send, un-issue and reverse are CA / SA only.** Sending allocates the gapless number and emails the customer — a company act, not a building act (**P-15 §0.1**, the owner-delegated answer to P-14's operator question). A BUILDING_MANAGER keeps the building-level half — drafts, preview, draft edits/lines, delete-draft, PDF, lists — and gets the refusal that names the next actor (stable code `invoice_admin_only`). | View gate [invoicing/views.py `_forbid_non_admin`](../../backend/invoicing/views.py) on `_transition` (the shared body of the four commit actions); independent re-checks in [invoicing/state_machine.py](../../backend/invoicing/state_machine.py) (`is_invoice_admin` in issue/send/unissue/reverse — Addendum B's double gate, keep both); the one helper + code in [invoicing/permissions.py](../../backend/invoicing/permissions.py). | [test_p15_h12_invoice_admin_gate.py](../../backend/invoicing/tests/test_p15_h12_invoice_admin_gate.py) |
