> **ARCHIVED — historical.** Describes the system as of 2026-05-28. Not maintained; do not rely on it. Live docs: `docs/README.md`.
...
| 18 | Staff completion routing (default BM review vs direct customer) | Per-staff / per-building toggle for `IN_PROGRESS → WAITING_MANAGER_REVIEW` vs `→ WAITING_CUSTOMER_APPROVAL` | `BuildingStaffVisibility.staff_completion_routes_to_customer` flag drives a routing-flag check inside `apply_transition`; the new `TicketStatus.WAITING_MANAGER_REVIEW` interstitial and four new `ALLOWED_TRANSITIONS` entries wire the STAFF default and configured routes; BM-review forward / reject legs are admin-only and BM rejection requires a non-empty note. Sprint 28 Batch 11 (backend). No new osius keys — per-row field + state-machine scope check is sufficient. | [`buildings/models.py:90-130`](../../backend/buildings/models.py#L90-L130), [`tickets/models.py:23-37`](../../backend/tickets/models.py#L23-L37), [`tickets/state_machine.py`](../../backend/tickets/state_machine.py) | OK | — | Closed by Sprint 28 Batch 11. Frontend completion modal still pending (separate dispatch). |
