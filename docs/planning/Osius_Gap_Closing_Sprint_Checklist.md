...
- [ ] **Phase 4b — provider "Facturen" UI** + the "who's due" list (driven by the billing schedule). ALSO removes the legacy Facturen page + the deprecated `/extra-work/mark-invoiced/` + `/clear-invoiced/` no-op endpoints (kept alive only for that page).
