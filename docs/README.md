...
| [product/role-visibility-matrix.md](product/role-visibility-matrix.md) | Role → navigation visibility matrix (the three Addendum D navigations + the customer-page tabs), every cell sourced from code (frontend gate **and** backend enforcement). Re-derived in FE-7 (2026-08-29). | Whenever the frontend nav gate or backend role scoping changes. |
...
| [planning/go-live-import.md](planning/go-live-import.md) | P-16 Part E4: the go-live data-import plan — what Ramazan's real records are, the dependency order, what the seed already proves, the questions for the owner. Plan only; the import sprint runs post-merge when the data exists. | At the data-handover meeting, and when the import sprint starts. |
| [planning/merge-recipe.md](planning/merge-recipe.md) | P-16 Part G: the one-command merge of the redesign train into `main` (tag `v2026.09-redesign`), what main gains, what merge does NOT do, and the branch cleanup a week later. Prepared; runs only on the owner's word. | If the train tip moves before the merge; delete after the merge completes. |
| [walkthrough/README.md](walkthrough/README.md) | P-16 Part F: one page per role (company admin, building manager, staff, customer), nl + en — what you see at login, the five things you do most, where the money is, what to do when something is stuck. Written for Ramazan; a stranger reads one page and can start. | When a role's home page or primary flow changes. |
| [reference/osius-reference-system/00-connection-map.md](reference/osius-reference-system/00-connection-map.md) | Read-only investigation of the **third-party Osius reference system** (Laravel + React), mapped as a connection graph centred on its Extra Work ↔ Invoicing spine. `00-connection-map.md` is the deliverable; `01`–`09` are the per-area evidence and the live-data / contradiction verification; `10` is the screen-by-screen walkthrough. Describes **their** system only — it is not a design for ours and prescribes nothing. | Only if the reference system is re-investigated. |
| [reference/osius-reference-system/10-screen-walkthrough.md](reference/osius-reference-system/10-screen-walkthrough.md) | Every screen in the reference system, in nav order, as **behaviour a user sees**: what each page is for, what each list shows and how it is filtered on landing, what each modal asks, and what the server actually enforces when the button is pressed. Written from its route table down, so it cannot miss a screen the way clicking around can. Exists so we stop discovering that system by accident. Each screen carries a `Ours:` line checked against our code (not against CLAUDE.md — §12.4 records the ten places the first pass got that wrong). Describes **their** system; prescribes nothing. | Only if the reference system is re-investigated. |
...
| [testing/capability-inventory.md](testing/capability-inventory.md) | P-14 (§D.24.6): every route, page, and interactive capability of the SPA — who may see it, what it does in one line, the server call it makes. The checklist the end-to-end audit ticks off; the report's first number is "N capabilities, M walked". | When a route or page-level capability is added, moved, or removed. |
| [testing/p14-findings.md](testing/p14-findings.md) | P-14's product: the end-to-end audit's findings list with evidence (page · role · what a stranger sees · severity S1–S4 · the fix sentence), items fixed in-sprint marked, the rest prioritised as P-15's input. | During P-14 and whenever a finding is fixed or re-triaged. |
| [testing/e2e-harness.md](testing/e2e-harness.md) | P-16 Part B: how to run the full Playwright suite against the dev stack in one sitting — preview proxy, throttle-raised second backend, the project-CLI-in-matching-image rule, cleanup. | When the harness (ports, image version, env vars) changes. |
| [testing/p16-suite-repair.md](testing/p16-suite-repair.md) | P-16 Parts A+B: the disposition of every pre-existing backend red and e2e red — repaired (code fixed), repinned (rule changed by a ruling, cited) or deleted (pinned nothing that exists), one line each. | Closed after P-16; a future red gets fixed in its own sprint, not appended here. |
| [testing/i18n-audit.md](testing/i18n-audit.md) | What `npm run i18n:audit`'s two numbers mean, with the current baseline (5286 checked · NS-FALLBACK 2 · MISSING 0), the one key that genuinely depends on `nsMode:"fallback"` and why, and the P-18 B2 finding that the old 58 was the script misattributing aliased `t` bindings rather than any misfiled key. | When either number moves, or the audit script's attribution changes. |
